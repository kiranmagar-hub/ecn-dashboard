...
CLEAN_PATH  = r'C:\Users\kmagar\OneDrive - Analog Devices, Inc\Documents\ECNMETRICS07022026\ECN_Metrics_2026-07-17\ECN_Metrics_Dashboard.html'
PROJ_PATH   = r'C:\Users\kmagar\Claude\projects\BEF ECN CT 0624 update\ECN_Metrics_Dashboard.html'
SP_PATH     = r'C:\Users\kmagar\Analog Devices, Inc\Backend FNDY Promis Group Site - Backend Foundry ECN Metrics\ECN_Metrics_Dashboard.html'

content = open(CLEAN_PATH, encoding='utf-8').read()
print('Clean file size:', len(content))
print('chart-mgmt-bar before:', content.count('chart-mgmt-bar'))
...
def wrap_chart(canvas_id, title_html, fw=False):
    ...
    return (
        f'<div class="chart-container{fw_class}" id="wrap-{canvas_id}" draggable="true">\n'
...
if overview_start != -1 and overview_end != -1:
    ...
    print('ERROR: Overview grid markers not found')
...
if kpi_start != -1 and kpi_end != -1:
    ...
    print('ERROR: KPI grid markers not found')

# 4. Inject JS before the LAST </body> (real HTML, not jsPDF template strings)
...
print('toggleChart count:', content.count('toggleChart'))
print('</body> count:', content.count('</body>'))
cm_pos = content.find('toggleChart')
body_pos = content.rfind('</body>')
print('CM before real body:', cm_pos < body_pos)

# Save to all 3 locations
open(CLEAN_PATH, 'w', encoding='utf-8').write(content)
shutil.copy2(CLEAN_PATH, PROJ_PATH)
shutil.copy2(CLEAN_PATH, SP_PATH)
print()
print('Saved to OneDrive source folder')
